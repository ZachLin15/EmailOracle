...
# Define file paths
nsb_file = f'C:\\Nestle\\NSBXTPLSH_{today_date}.csv'
global xn_file
folder = r"C:\Users\USER\OneDrive\XN report"
for file in os.listdir(folder):
    if file.__contains__("xlsx"):
        xn_file = os.path.join(folder,file)
...
def ProcessData():
    ...
    nsb_file_copy['EXPDATE'] = result_df['expiry_date'].values
    nsb_file_copy = nsb_file_copy.fillna('')
    nsb_file_copy['EXPDATE'] = pd.to_datetime(nsb_file_copy['EXPDATE'],errors='coerce')
    nsb_file_copy['EXPDATE'] = nsb_file_copy['EXPDATE'].dt.strftime('%d%m%y')
    nsb_file_copy = nsb_file_copy.fillna('')

    nsb_file_copy['EXPDATE'] = "'" + nsb_file_copy['EXPDATE'].astype('str')
    nsb_file_copy.drop('new_column', axis=1, inplace=True)

    nsb_file_copy.to_csv(nsb_file,index=False)
    print("sdf")
